    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
          className="text-sm"
          style={{ color: i <= count ? "var(--color-primary)" : "#333" }}
        >
          ★
        </span>
      ))}
...
function TestimonialCard({ t }: { t: (typeof config.testimonials)[number] }) {
  return (
    ...
    >
      ...
      <p className="font-cairo text-sm leading-relaxed opacity-80 flex-1">
        "{t.text}"
      </p>
  ...
  return (
    ...
    >
      <div className="px-4 mb-6 flex items-center gap-4">
        ...
        />
        <h2 className="font-bebas text-3xl tracking-widest">
