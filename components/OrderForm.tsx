...
const BTN_BASE =
  "w-full flex items-center justify-center gap-2 font-cairo font-bold text-base rounded-xl transition-all active:scale-95 hover:opacity-90";
...
export default function OrderForm({ onSubmitSuccess }: OrderFormProps) {
  ...
  const validate = () => {
    ...
    if (!form.phone.trim() || !/^0[5-7]\d{8}$/.test(form.phone))
  ...
  const handleSubmit = () => {
    ...
    setForm({ name: "", phone: "", wilaya: "", size: "", color: "" });
    setErrors({});
    onSubmitSuccess?.();
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <p className="font-cairo text-xs opacity-50">
          🚚 التوصيل في {config.deliveryTime} لكل الولايات
        </p>
      </div>
      ...
      {config.stockCount > 0 && (
        ...
        >
          <span>⚡</span>
          <p className="font-cairo text-sm text-red-400">
            تبقاو غير {config.stockCount} قطع — اطلب قبل ما يخلص
          </p>
        </div>
      )}
      ...
      {/* Name */}
      <div>
        <label className="font-cairo text-xs opacity-60 block mb-2">
          الاسم الكامل *
        </label>
        <input
          type="text"
        ...
        />
        {errors.name && (
          <p className="font-cairo text-xs text-red-400 mt-1">{errors.name}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.phone && (
          <p className="font-cairo text-xs text-red-400 mt-1">{errors.phone}</p>
        )}
      </div>
      ...
      <div>
        ...
        {errors.wilaya && (
          <p className="font-cairo text-xs text-red-400 mt-1">
            {errors.wilaya}
          </p>
        )}
      </div>
      ...
      <div>
                ...
                borderColor:
                  form.size === s
                    ...
                    : errors.size
                      ...
                      : "var(--color-border)",
                backgroundColor:
                  form.size === s ? "var(--color-primary)" : "transparent",
        ...
        {errors.size && (
          <p className="font-cairo text-xs text-red-400 mt-1">{errors.size}</p>
        )}
      </div>
      ...
      <div>
                ...
                borderColor:
                  form.color === c
                    ...
                    : errors.color
                      ...
                      : "var(--color-border)",
                backgroundColor:
                  form.color === c ? "rgba(200,241,53,0.1)" : "transparent",
                color:
                  form.color === c
                    ? "var(--color-primary)"
                    : "var(--color-text)",
        ...
        {errors.color && (
          <p className="font-cairo text-xs text-red-400 mt-1">{errors.color}</p>
        )}
      </div>
